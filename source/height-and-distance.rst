...
140. A balloon is observed simultaneously from the points :math:`A, B, C` on straight road directly beneath it. The angular
...
141. A man on a cliff observes a boat at an angle of depression of :math:`30^\circ`, which is sailing towards the shore to the
...
142. An aeroplane flying horizontally (at a constant speed) :math:`1` km above the ground is observed at an elevation of
     :math:`60^\circ`. If after :math:`10` second, the elevation be :math:`30^\circ`, find the spped of the aeroplane in km/hr.

143. An aeroplane when :math:`3000` m high passes vertically above another aeroplane at an instant when there angle of elevation at
     the same observation points are :math:`60^\circ` and :math:`45^\circ` respectively. How many meters higher is the one than the
     other.

144. The angles of elevation of an aeroplane at two consecutive milestones respectively are :math:`\alpha` and :math:`\beta`. Find
     the height of the plane taking it to be between the two milestones and just abobve the road.

145. The altitude of a certain rock is :math:`47^\circ` and after walking towards it :math:`1000` m up a slope inclined at
...
146. A man observes that when he moves up a distance :math:`c` m on a slope, the angle of depression of a point on the horizontal
...
147. On level ground the angle of elevation of the top of the tower is :math:`30^\circ`. On moving :math:`20` m nearer the angle of
     elevation is :math:`60^\circ`. What is the height of the tower?

148. An air-pilot at a height :math:`h` m above the ground observes the angle of depression of the top and bottom of a tower to be
     :math:`30^\circ` and :math:`60^\circ`. Find the height of the tower.

149. From the top of a hill :math:`200` m high, the angles of depression of the top and the bottom of a pillar are :math:`30^\circ`
     and :math:`60^\circ` respectively. Find the height of the pillar and its distance from the hill.

150. A vertical pole consists of two parts, the lower part being one-third of the whole. The upper part subtends an angle whose
     tangent is :math:`\frac{1}{2}` at a point in a horizontal plane through the foot of the pole and :math:`20` m from it. Find
     the height of the pole.

151. A statue is :math:`8` m high standing on the top of a tower :math:`64` m high on the bank of a river subtends at a point
...
152. A statue :math:`a` m high placed on a column :math:`b` m high subtends the same angle as the column to an observer :math:`h` m
...
153. A tower subtends an angle :math:`\alpha` at a point :math:`A` on the same level as the foot of the tower. :math:`B` is a point
...
154. The angles of elevation of the top of a tower standing on a horizontal plane from two points on a line passing through the foot
...
155. A pillar subtends at a point :math:`d` m apart from its foot the same angle as that subtended at the same point by a statue on
     the top. If the pillar is :math:`h` m high, show that the height of the status is :math:`\frac{b(d^2 + h^2)}{d^2 - h^2}` m.

156. A vertical tower :math:`50` ft. high stands on a sloping ground. The foot of the tower is at the same level as the middle
...
157. An observer at an anti-aircraft post :math:`A` identifies an enemy aircraft due east of his post at an angle of elevation of
...
158. A flag staff :math:`PN` stands up right on level ground. A base :math:`AB` is measured at right angled to :math:`AN` such that
...
159. A vertical pole is divided in the ratio :math:`1:9` by a mark on it. If the two parts subtend equal angle at a distance of
     :math:`20` m from the base of the pole, find the height of the pole. The lower part is shorter than the upper one.

160. A chimney leans towards norht. At equal distancces due north and south of it in a horizontal plane, the elevation of the top
...
161. A flag staff :math:`10` m high stands in the center of an equilateral triangle which is horizontal. If each side of the
     triangle subtends an angle of :math:`60^\circ` at the top of flag staff. Prove that the length of the sides are
     :math:`5\sqrt{6}` m.

162. Two posts are :math:`120` m apart, and the height of one is double that of the other. From the middle point of the line
...
163. A pole :math:`100` ft. high stands at the center of an equilateral triangle each side of which subtends and angle of
     :math:`60^\circ` at the top of the pole. Find the side of the triangle.

164. An observer on a carriage moving with a speed :math:`v` along a straight road observes in one position that two distant trees
...
165. :math:`A` and :math:`B` are two points on one bank of a straight river and :math:`C` and :math:`D` are two points on the other
...
166. To measure the breadth :math:`PQ` of a river a man places himself at :math:`R` in the straight line :math:`PQ` produced
...
167. A bird is perched on the top of a tree :math:`20` m high and its elevation from a point on the ground is :math:`45^\circ`. It
...
168. The angles of elevation of a balloon from two stations :math:`2` km apart and from a point halfway between them are observed
     to be :math:`60^\circ, 30^\circ` and :math:`45^\circ` respectively. Prove that the height of the balloon is
     :math:`500\sqrt{6}` m.

169. If the angular elevations of the tops of two spires which appear in a straight line is :math:`\alpha` and the angular
...
170. A pole stands vertically on the center of a square. When :math:`\alpha` is the elevation of the sun its shadow just reaches
...
171. A circular plate of radius :math:`a` touches a vertical wall. The plate is fixed horizontally at a height :math:`b` above the
...
172. The extremity of the shadow of a flag-staff which is :math:`6` m high and stands on the top of a pyramid on a square base,
...
173. A man observes a tower :math:`AB` of height :math:`h` from a point :math:`P` on the ground. He moves forward a distance
...
174. A man observes that when he moves up a distance :math:`c` m on a slope, the angle of depression of a point on the horizontal
...
175. A :math:`2` m long object is fired vertically upwards from thr mid-point of two locations :math:`A` and :math:`B, 8` m
...
176. A sign-post in the fom of an isosceles triangle :math:`ABC` is mounted on a pole of height :math:`h` fixed to the
...
177. A tower is observed from two stations :math:`A` and :math:`B`, where :math:`B` is east of :math:`A` at a distance
...
178. Two vertical poles whose heights are :math:`a` and :math:`b` subtend the samme angles :math:`\alpha` at a point in the line
...
179. :math:`PQ` is a vertical tower. :math:`P` is the foot and :math:`Q` is the top of the tower. :math:`A, B, C` are three points
...
180. An observer at :math:`O` notices that the angle of elevation of the top of a tower is :math:`90^\circ`. The line joining
...
181. A tower :math:`AB` leans towards west making an angle :math:`\alpha` with the vertical. The angular elevation of :math:`B`,
...
182. The elevation of the top of a tower at point :math:`E` due east of the tower is :math:`\alpha`, and at a point :math:`S` due
...
183. A vertical tree stands at a point :math:`A` on a bank of a canal. The angle of elevation of its top from a point :math:`B` on
...
184. A person observes the top of a vertical tower of height :math:`h` from a station :math:`S_1` and finds :math:`\beta_1` is the
...
185. A straight pillar :math:`PQ` stands at a point :math:`P`. The points :math:`A` and :math:`B` are situated due north due south
...
186. :math:`ABC` is a triangular park with :math:`AB = AC = 100` m. A television tower stands at the mid point of :math:`BC`. The
...
187. A square tower stands upon a horizontal plane from which three of the upper corners are visible, their angular elevations are
...
188. A right circular cylindrical tower of height :math:`h` and radius :math:`r` stands on a horizontal plane. Let :math:`A` be a
...
189. A is the foot of the vartical pole, :math:`B` and :math:`C` are due east of :math:`A` and :math:`D` is due south of
...
190. A person wishing to ascertain the height of a tower, stations himself on a horizontal plane through its foot at a point at
...
191. A tower stands in a field whose shape is that of an equilateral triangle and whose side is :math:`80` ft. It subtends angles
     at three corners whose tangents are respectively :math:`\sqrt{3} + 1, \sqrt{2}, \sqrt{2}`. Find its height.

192. A flag-staff on the top of a tower is observed to subtend the same angle :math:`\alpha` at two points on a horizontal plane,
...
193. A man standing on a plane observes a row of equal and equidistant pillars, the :math:`10`-th and :math:`17`-th of which
...
194. A tower stands on the edge of the circular lake :math:`ABCD`. The foot of the tower is at :math:`D` and the angle of elevation
...
195. A pole stands at the bank of circular pond. A man walking along the bank finds that angle of elevation of the top of the pole
...
196. A man standing on the sea shore observes two buoys in the same direction, the line through them making an angle :math:`\alpha`
...
197. A railway curve in the shapre of a quadrant of a circle, has :math:`n` telegraph posts at its ends and at equal distance along
...
198. A wheel with diameter :math:`AB` touches the horizontal ground at the point :math:`A`. There is a rod :math:`BC` fixed at
...
199. A semi-circular arch :math:`AB` of length :math:`2L` and a vertical tower :math:`PQ` are situated in the same vertical
...
200. A circle passes through three points :math:`A, B` and :math:`C` with the line segment :math:`AC` as its diameter. A line
...
201. The angle of elevation of a cloud from a point :math:`h` m above a lake is :math:`\alpha`, and the angle of depression of its
     reflection is :math:`\beta`. Prove that the distance of the observer from the cloud is :math:`\frac{2h\cos\beta}{\sin(\beta -
     \alpha)}`.

202. An isosceles triangle of wood is placed in a vertical plane, vertex upwards and faces the the sun. If :math:`2a` be the base
...
203. A rectangular target faces due south, being vertical and standing on a horizontal plane. Computer the area of the target with
     that of its shadow on the ground when the sun is :math:`\beta^\circ` from the south at an altitude of :math:`\alpha^\circ`.

204. The extremity of the shadow of a flag staff which is :math:`6` m high and stands on the top of a pyramid on a square base just
...
205. The shdadow of a tower is observed to be half the known height of the tower and sometime afterwards is equal to the known
...
206. A man notices two objects in a straight line due west. After walking a distance :math:`c` due north, he observes that the
...
207. A stationary balloon is observed from three points :math:`A, B` and :math:`C` on the plane ground and it is found that its
     angle of elevation from each of these points is :math:`\alpha`. If :math:`\angle ABC = \beta` and :math:`AC = b`, find the
     height of the balloon.

208. A light house, facing north, sends out a fan-shaped beam of light extending from north-east to north-west. An observer on a
...
209. A man walking due north observes that the elevation of a balloon, which is due east of him and is sailing tpwards the
...
210. A flag-staff stands on the middle of a square tower. A man on the ground opposite the middle of the face and distant from it
...
211. A vertical pole stands at a point :math:`O` on horizontal ground. :math:`A` and :math:`B` are points on the ground, :math:`d`
...
212. A vertical tree stands on a hill side that makes an angle :math:`\alpha` with the horizontal. From a point directly up the
...
213. A person stands on the diagnal produced of the square base of a church tower, at a distance :math:`2a` from it and observes
...
214. The elevation of a steeple at a place due south of it is :math:`45^\circ` and at another place due west of the former place is
...
215. A tower surmouned by a spire stands on a level plane. A person on the plain observes that when he is at a distance :math:`a`
...
216. From the bottom of a pole of height :math:`h`, the angle of elevation of the top of the tower is :math:`\alpha`. The pole
     subtends angle :math:`\beta` at the top of the tower. Find the height of the tower.

217. A man moves along the bank of a canal and observes a tower on the other bank. He finds that the angle of elevation of the top
...
218. The angle of elevation of a balloon from two stations :math:`2` km apart and from a point halfway between them are observed to
     be :math:`60^\circ, 30^\circ` annd :math:`45^\circ` respectively. Prove that the height of the balloon is :math:`500\sqrt{5}`
     meters.

219. A flag staff :math:`10` meters high stands in the center of an equilateral triangle which is horizontal. If each side of the
...
220. A tower standing on a cliff subtends an angle :math:`\beta` at each of two stations in the same horizontal line passing
...
221. A man walking towards a tower :math:`AB` on which a flag staff is fixed observes that when he is at a point :math:`E`,
...
222. An observer on a carriage moving with a speed :math:`v` along a straight-road observes in one position that two distant trees
...
223. Four ships :math:`A, B, C` and :math:`D` are at sea in the following positions. :math:`B` is on a straight line segment
...
224. A train is moving at a constant speed at an angle :math:`\theta` east of north. Observations of the train are made from a
     fixed point. It is due north at some instant. Ten minutes earlier its bearing is :math:`\alpha_2` east of north. Find
     :math:`\tan\theta`.

225. A man walks in a horizontal circle round the foot of a flag staff, which is inclined to the vertical, the foot of the flag
...
226. A bird flies in a circle on a horizontal plane. An observer stands at a point on the ground. Suppose :math:`60^\circ` and
...
227. A hill on a level plane has the form of a portion of a sphere. At the bottom the surface slopes at an angle :math:`\alpha` and
...
228. A hill standing on a horizontal plane, has a circular base and forms a part of a sphere. At two points on the plane, distant
...
229. On the top of a hemispherical dome of radius :math:`r` there stands a flag of height :math:`h`. From a point on the ground the
...
230. A man walks on a horizontal plane a distance :math:`a`, then through a distance :math:`a` at an angle :math:`\alpha` with his
...
231. In order to find the dip of a stream of coal below the surface of the ground, vertical borings are made from the angular point
...
232. Two lines of rails make an angle of :math:`75^\circ` with one another. They are to be connected by a circular arc of central
...
233. A gun is fired from a fort :math:`F` at a distance :math:`d` from a station :math:`O` and from two stations :math:`A` and
...
234. A tunnel is to be bored from :math:`A` to :math:`B`, which are two places on the opposite sides of a mountain. From :math:`A`
...
235. An observer looking up the line of greatest slope of an inclined plane sees a vertical tower due east of him. He walks
...
236. Observations made to estimate the radius of the moon have shown that the semi-vertical angle of the tangential cone, drawn
...
237. :math:`A, B` and :math:`C` are three consecutive milestones on a straight road from each of which a distant spire is
...
238. :math:`A, B` and :math:`C` are the tops of posts of equal height at equal intervals by the side of a road and :math:`t` and
...
239. A boy standing :math:`C` meters behind and directly opposite the center of a football goal observes the angle of elevation of
...
240. A man walks in a horizontal circle round the foot of a pole which is inclined to the vertical. The foot of the pole is at the
...
241. A bridge has :math:`5` equal spans, each of :math:`10` m, measured from the center of the piers, and a boat is moved in a line
...
242. A person standing by rge side of a road observesa row of equidistant telephone poles of equal height. Neglecting the height of
